...
function openDialog(row = null) {
  ...
  form.value = row
    ...
    : { template_type: types.value[0] || '', mode: modes.value[0] || '', content: '', sort_order: 0, is_enabled: true }
...
async function save() {
  try {
    ...
    if (form.value.id) {
      await cmsTemplateApi.update(form.value.id, payload)
    } else {
      await cmsTemplateApi.create(payload)
    }
