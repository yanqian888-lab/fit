...
async function callHelperAgent(question, userInfo = {}, partnerInfo = {}, options = {}) {
  ...
  const needsFoodData = /(吃|喝|食物|酸奶|饭|菜|肉|水果|饮料|晚餐|午餐|早餐|加餐|零食|热量|卡路里|千卡|摄入|吃了多少|总计|汇总|算|脂肪|蛋白质|碳水|营养|鱼|虾|蟹|贝|海鲜|三文鱼|鳗鱼|金枪鱼|榴莲|芒果|菠萝|凤梨|西红柿|番茄|土豆|马铃薯|地瓜|番薯|奇异果|车厘子|桔子|元宵|雪糕|冰淇淋|冰激凌|奶茶|咖啡|豆浆|披萨|汉堡|薯条|炸鸡|火锅|麻辣烫|烧烤|泡面|方便面|螺蛳粉|米线|米粉|沙拉|寿司|麦片|蛋卷|麻花|桃酥|威化|麻薯|卤蛋|果汁)/.test(question);
...
function shouldUseWebSearch(question, unknownFood) {
  ...
  const snackRe = /雪糕|冰淇淋|冰激凌|冰棍|棒冰|冰棒|甜筒|圣代|雪泥|薯片|薯条|虾条|锅巴|爆米花|沙琪玛|雪饼|米饼|月饼|烧饼|煎饼|蛋糕|年糕|面包|吐司|饼干|曲奇|蛋挞|蛋塔|泡芙|披萨|汉堡|巧克力|棉花糖|太妃糖|麦芽糖|软糖|奶糖|果冻|辣条|肉干|肉脯|豆干|香肠|火腿肠|凤爪|鸡爪|鸭脖|鸭翅|海苔|鱼片|话梅|蜜饯|瓜子|花生|核桃|腰果|开心果|巴旦木|松子|板栗|红枣|桂圆|蛋卷|春卷|威化|麻花|桃酥|雪花酥|牛轧糖|甜甜圈|麻薯|麻糬|士力架|山楂片|牛肉干|鱿鱼丝|卤蛋/;
...
function collectFoodCandidates(question) {
  ...
  // 设计要点：
  // - 前缀 0~8 字：品类名本身（"冰淇淋""辣条"）可整词命中；品牌+口味+品类组合（"索菲亚朗姆口味冰淇淋"）
  //   靠前缀捕获，4 字不够（会截断品牌成"朗姆口味"），8 字可容纳"索菲亚朗姆口味"，前导助词在清洗段剥除
  // - 只列足够具体的后缀，禁用裸字（如裸"糖"会误抓"血糖"、裸"糕"会误抓"糟糕"、裸"饼"会误抓"铁饼"）；
  //   "冰激凌"（激）与"冰淇淋"（淇）是常见变体写法，缺一个会整句漏提取（生产实测教训）
  // - 个别日常歧义词（面包车/脑瓜子）漏网后由 searchAndLearnFood 四重校验兜底，最坏浪费一次检索
  const snackRe = /([\u4e00-\u9fa5]{0,8}(?:雪糕|冰淇淋|冰激凌|冰棍|棒冰|冰棒|甜筒|圣代|雪泥|薯片|薯条|虾条|锅巴|爆米花|沙琪玛|雪饼|米饼|月饼|烧饼|煎饼|蛋糕|年糕|面包|吐司|饼干|曲奇|蛋挞|蛋塔|泡芙|披萨|汉堡|巧克力|棉花糖|太妃糖|麦芽糖|软糖|奶糖|果冻|辣条|肉干|肉脯|豆干|香肠|火腿肠|凤爪|鸡爪|鸭脖|鸭翅|海苔|鱼片|话梅|蜜饯|瓜子|花生|核桃|腰果|开心果|巴旦木|松子|板栗|红枣|桂圆|蛋卷|春卷|威化|麻花|桃酥|雪花酥|牛轧糖|甜甜圈|麻薯|麻糬|士力架|山楂片|牛肉干|鱿鱼丝|卤蛋))/g;
  ...
  // 兜底：纯商品名/品牌名（如"聪明转转""大冰桶""椰子灰"）不含品类后缀词、上述正则全部未命中时，
  // 若问句本身是热量/含糖类咨询，则剥离疑问词与句式噪声提取问句主语作为候选。
  // 剥离结果须通过黑名单校验（代词/疾病/动作词等），防止把闲聊句误当食物名；误触发最多浪费一次联网检索（四重校验兜底）。
  if (cleanedList.length === 0
    && /(热量|卡路里|千卡|大卡|含糖|能吃|能喝|可以吃|可以喝|会胖|长胖|多少卡|几卡|想吃|想喝)/.test(question)) {
    const stripped = question
      .replace(/[？?！!，。,.\s]/g, ' ')
      // 分数/约数量词：四分之一索菲亚朗姆口味冰激凌（生产实测案例，不剥离会导致整段超长被丢弃）
      .replace(/(二分之一|三分之一|四分之一|八分之一|十六分之一|半个|半份|半盒|半个|半杯|半瓶|一口)/g, ' ')
      .replace(/(热量是多少|热量高不高|热量高不|热量高吗|热量多少|热量怎么样|热量高|热量低|热量|卡路里|千卡|大卡|多少卡|几卡|含糖量|含糖|会不会胖|会胖吗|会胖|能不能吃|能不能喝|可以吃吗|可以喝吗|能吃吗|能喝吗|能吃|能喝|可以吃|可以喝|好吃吗|好喝吗|多少钱|想吃|想喝|来个|来杯|吃个|喝个|今天|明天|昨天|上午|下午|中午|晚上|半夜|早上|现在|一个|一根|一块|一包|一盒|一瓶|一杯|一支|一条|一袋|一碗|一份|那是|就|想|要|问|查|看|算|帮|给|说|和|跟|那|呢|吗|啊|哦|吧|嘛|呀|了|的|高|胖)/g, ' ')
      .trim();
    for (const seg of stripped.split(/\s+/)) {
      // 长度上限 12：容纳"索菲亚朗姆口味冰激凌"（11字）这类品牌+口味+品类完整组合
      if (seg.length < 2 || seg.length > 12) continue;
      // 黑名单：代词/身体疾病词/与健康咨询相关的动作词，命中则跳过该片段
      if (/(我|你|他|她|它|血糖|血压|糖尿|痛风|体重|身高|运动|锻炼)/.test(seg)) continue;
      if (!seen.has(seg)) {
        seen.add(seg);
        cleanedList.push(seg);
      }
    }
  }
  return cleanedList;
