...
const FOOD_KEYWORDS = [
  ...
  '咖啡', '坚果', '蔬菜', '主食', '玉米', '红薯', '地瓜', '番薯', '紫薯', '土豆', '马铃薯',
  '西红柿', '番茄', '黄瓜', '山药', '芋头', '莲藕', '萝卜', '胡萝卜', '冬瓜', '南瓜',
  '丝瓜', '苦瓜', '茄子', '辣椒', '韭菜', '芹菜', '菠菜', '白菜', '娃娃菜', '豆角',
  '豆芽', '西兰花', '花菜', '蘑菇', '香菇', '金针菇', '杏鲍菇', '笋',
  '苹果', '橙子', '橘子', '桔子', '葡萄', '西瓜', '草莓', '蓝莓', '猕猴桃', '奇异果',
  '牛油果', '奶酪', '芝士', '榴莲', '木瓜', '菠萝', '凤梨', '山竹', '火龙果',
  '百香果', '椰子', '哈密瓜', '香瓜', '杨梅', '枇杷', '柿子', '柚子', '梨', '桃子',
  '李子', '杏', '枣', '龙眼', '桂圆', '车厘子', '樱桃',
  '鱼', '虾', '蟹', '贝', '生蚝', '扇贝', '鱿鱼', '三文鱼', '鳗鱼', '金枪鱼',
  '肉包', '水饺', '馄饨', '肠粉', '粥', '粉', '面', '饵块', '年糕', '汤圆', '元宵', '粽子',
  '泡面', '方便面', '螺蛳粉', '米线', '米粉', '麻辣烫', '火锅', '烧烤', '炸鸡',
  '披萨', '汉堡', '三明治', '寿司', '刺身', '沙拉', '麦片',
  ...
  '蛋卷', '蛋挞', '蛋塔', '泡芙', '马卡龙', '牛轧糖', '雪花酥', '沙琪玛', '麻花', '馓子',
  '江米条', '萨其马', '糕点', '甜点', '甜品', '糖水', '甜汤', '奶茶', '奶盖',
  '烧仙草', '芋圆', '西米', '红豆', '绿豆', '薏仁', '紫米', '黑米', '糯米',
  '麻糬', '麻薯', '芋泥', '地瓜泥', '南瓜泥', '紫薯泥', '山药泥', '土豆泥', '果酱',
  '果冻', '冰淇淋', '冰激凌', '雪糕', '冰棍', '冰棒', '圣代', '奶昔', '思慕雪', '果汁',
...
function isCasualChat(content) {
  ...
  // 1. 纯标点、语气词、笑声、表情符号占位
  if (/^[？?！!，,。.．·…~～\s哈呵嘿哼嗯哦啊呀哇呜诶嘛呢呗]+$/i.test(text)) return true;

  // 2. 常见纯短句/语气回应（不含数字、计量单位、具体食物/运动名词）
  // 注意："个"不作为计量单位过滤，否则"想喝个奶茶"会被误判
  const shortText = text.length <= 16 && !/\d/.test(text) && !/(克|g|公斤|kg|千卡|大卡|kcal|分钟|公里|步|片|碗|杯|勺|根|块|包|袋|瓶|ml|毫升|升|l\b)/i.test(text);
  if (shortText) {
  ...
  const casualPatterns = [
    ...
    /^(随便|都行|可以的|好的|OK|ok|嗯|哦|啊|好|行|可以|知道了|明白|了解|就这样|算了|罢了|摆烂|躺平)[吧啊呢]?$/i,
    /^(哈哈哈哈|哈哈哈|呵呵呵|嘿嘿|哼哼|呜呜呜|呜呜|嘤嘤嘤|救命|笑死|栓q|栓Q|6|666|芜湖|绝了|牛逼|卧槽|靠|天呐|天啊)$/i,
    /^(怎么|为什么)(这样|回事|会这样|这么|总是|老是)[啊呢]?$/,
    /^(就这样吧|那好吧|好吧|行吧|算了算了|罢了罢了|爱咋咋地|随便吧|无所谓)$/,
    /^(晚安|拜拜|再见|see you|goodnight|good night|拜拜了)$/i
  ];
  if (casualPatterns.some(p => p.test(text))) return true;
 ...
 * 约束：必须是疑问/求助/明确意向句式，且命中专业关键词，避免情绪化闲聊被误判
 */
function isProfessionalQuestion(content) {
  if (!content || isCasualChat(content)) return false;

  const text = String(content).trim();
  // 必须是疑问/求助/明确意向句式，才值得调用 Helper 给精确数据
  const isAskingOrRequesting = /[?？吗嘛呢怎么办如何怎么为什么推荐建议适合].*?$/.test(text)
    || /^(帮我|给我|求|请问|想知道|想问问|想了解一下|给我个|帮我算|帮我算一下|帮我看看|帮我分析).+/.test(text)
    || /^(这个|那个|这样|那样).*[多少|吗|呢|吗嘛|行不|好不|可以不|能不能].*?$/.test(text);
  if (!isAskingOrRequesting) return false;

  const professionalKeywords = [
    '热量', '代谢', 'BMI', '基础代谢', '热量缺口', '碳蛋脂', '营养', '食谱',
    '运动', '为什么不掉秤', '平台期', '怎么瘦', '吃什么', '怎么吃', '推荐', '建议',
    '适合', '方法', '技巧', '如何', '怎样', '做什么', '减肥', '减脂', '增肌', '塑形',
    '卡路里', '蛋白质', '脂肪', '碳水', '健身', '训练', '有氧', '无氧', '拉伸',
    '瑜伽', '跑步', '游泳', '跳绳', 'HIIT', 'Tabata', '深蹲', '平板支撑', '俯卧撑',
    '健康', '体重', '体脂', '围度', '腰围', '腿围', '臀围', '胸围',
    '代餐', '蛋白粉', '补剂', '维生素', '矿物质', '膳食纤维',
    '升糖', '血糖', '胰岛素', '低碳', '生酮', '地中海', '轻食',
    '暴食', '节食', '断食', '轻断食', '168', '便秘', '水肿'
  ];
